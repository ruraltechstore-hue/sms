    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-xl border bg-card p-4 text-center">
          ...
          <p className="font-bold text-lg text-success">₹{totalPaid.toLocaleString("en-IN")}</p>
        </div>
        <div className="rounded-xl border bg-card p-4 text-center">
          <AlertTriangle className="h-5 w-5 mx-auto text-warning mb-1" />
          <p className="text-xs text-muted-foreground">Outstanding</p>
          <p className="font-bold text-lg text-warning">₹{totalDue.toLocaleString("en-IN")}</p>
        </div>
        <div className="rounded-xl border bg-card p-4 text-center">
          <CheckCircle className="h-5 w-5 mx-auto text-primary mb-1" />
          <p className="text-xs text-muted-foreground">Fee Status</p>
          <p className={`font-bold text-lg capitalize ${child.feeStatus === "paid" ? "text-success" : "text-warning"}`}>{child.feeStatus}</p>
        </div>
      </motion.div>
            ...
            return (
              <div key={fee.id} className="rounded-xl border p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-secondary/30 transition-colors">
                <div className="flex items-center gap-3">
                  <div className={`h-9 w-9 rounded-lg flex items-center justify-center ${cfg.style}`}>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    ...
                    {fee.paidAmount > 0 && fee.paidAmount < fee.amount && (
                      <p className="text-xs text-muted-foreground">Paid: ₹{fee.paidAmount.toLocaleString("en-IN")}</p>
                    )}
                  </div>
                  <span className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${cfg.style}`}>
                    {fee.status}
                  </span>
                </div>
              </div>
