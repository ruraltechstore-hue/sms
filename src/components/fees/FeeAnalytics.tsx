...
import { MONTHLY_COLLECTIONS, CLASS_WISE_DUES, FEE_PAYMENTS } from "@/lib/mock-fees";

const COLORS = ["hsl(var(--success))", "hsl(var(--warning))", "hsl(var(--destructive))", "hsl(var(--muted-foreground))"];

export function FeeAnalytics() {
  const statusCounts = FEE_PAYMENTS.reduce((acc, p) => {
  ...
  const collectionData = MONTHLY_COLLECTIONS.map((m) => ({
    ...m,
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {CLASS_WISE_DUES.map((row) => {
                const pct = Math.round((row.collected / row.total) * 100);
                return (
