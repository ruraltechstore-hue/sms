...
const leaveTypeStyles: Record<string, string> = {
  casual: "bg-accent/10 text-accent", sick: "bg-destructive/10 text-destructive",
  earned: "bg-primary/10 text-primary", maternity: "bg-warning/10 text-warning", unpaid: "bg-muted text-muted-foreground",
};
...
export function LeaveTracker() {
  ...
  useEffect(() => {
    const fetchData = async () => { const d = await staffService.getLeaveRecords(); setLeaves(d); setLoading(false); };
    fetchData();
  }, []);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border bg-card p-6">
      ...
      {filtered.length === 0 ? (
        <EmptyState title="No Leave Records" description={leaves.length === 0 ? "Leave records will appear here." : "No records match your filter."} />
      ) : (
              ...
              <motion.div key={leave.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div><h4 className="font-semibold text-sm">{leave.staffName}</h4><p className="text-xs text-muted-foreground">{leave.department}</p></div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium capitalize ${leaveTypeStyles[leave.leaveType] || ""}`}>{leave.leaveType}</span>
