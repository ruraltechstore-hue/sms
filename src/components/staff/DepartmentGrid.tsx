...
import { DEPARTMENTS } from "@/lib/mock-staff";
...
function Section({ title, departments }: { title: string; departments: typeof DEPARTMENTS }) {
  return (
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <Badge variant="outline" className="text-[10px] capitalize">{dept.type}</Badge>
              </div>
            </div>
            <div className="space-y-1 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">HOD</span>
                <span className="font-medium text-xs">{dept.hod}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground">Staff</span>
                <span className="flex items-center gap-1 font-semibold">
                  <Users className="h-3 w-3" />{dept.staffCount}
                </span>
              </div>
            </div>
          </motion.div>
